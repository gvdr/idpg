...
"""
Internal struct to wrap a time-varying intensity at a fixed time t.
...
function sample_ppp_temporal(ρ::TimeVaryingIntensity{d}, t_start::Real, t_end::Real;
    ...
    while t < t_end
        ...
        # Create a temporary intensity wrapper for time t
        frozen_ρ = FrozenTimeIntensity{d, typeof(ρ.ρ)}(ρ.ρ, t)

        # We need to estimate max intensity for this time slice
        # Since t changes slowly, maybe we could reuse λ_max?
        # For safety, re-estimate.
