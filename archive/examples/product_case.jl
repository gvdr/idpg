...
# - E[N] = c_G · c_R
# - E[|E|] = E[N]² · (μ̃_G · μ̃_R)    [node-centric: N sites → N² pairs]
# - E[L] = (E[N]/2) · (μ̃_G · μ̃_R)   [edge-centric: E[N]/2 opportunities]
# - E[|E|] / E[L] = 2 × E[N]
#
# Key insight: In edge-centric, each edge opportunity "consumes" 2 node-equivalents
# (1 source + 1 target), so from E[N] nodes we get E[N]/2 opportunities.
...
println("=" ^ 60)
println("Node-centric: E[|E|] = E[N]² × E[g·r]")
println("Edge-centric: E[L] = (E[N]/2) × E[g·r]")
println("Ratio: E[|E|]/E[L] = 2×E[N]")
println("=" ^ 60)
...
for (i, case) in enumerate(test_cases)
    ...
    for _ in 1:n_trials
        ...
        ec = sample_edge_centric(ei; rng=rng)
    ...
    println("\nRatio E[|E|]/E[L] = 2×E[N]:")
    if mean(L_samples) > 0
        println("  Theory:    ", round(2 * stats.E_N, digits=2))
    ...
    avg_degree_empirical = mean(E_samples) / max(1.0, mean(N_samples))
