...
#
# This example uses the "encounter model" for edge-centric sampling:
# - Sample N sites from PPP, each site (g, r) is an encounter opportunity
# - Each encounter becomes a predation with probability g · r
# - Expected edges: E[L] ≈ E[N] × E[g·r]
#
# For the alternative "pairing model" where E[N]/2 opportunities pair source and target,
# see sample_edge_centric() and SymmetricEdgeIntensity.
...
#
# Under the encounter interpretation, each sampled point (g, r) represents
# a potential trophic interaction: an encounter that becomes predation with prob g · r.
#
# Note: In this model, each site has its own g and r - representing an individual
# that could be either a resource (eaten based on g) or consumer (eating based on r).
...
println("\n" * "=" ^ 60)
println("Monte Carlo Comparison (Encounter Model)")
println("=" ^ 60)
println("Using: E[L] = E[N] × E[g·r] (each site is an encounter opportunity)")
...
for (idx, scenario) in enumerate(scenarios)
    ...
    println("\n", scenario.name, ":")
    println("  E[N]: ", round(stats.E_N, digits=1))
    println("  E[L] = E[N] × E[g·r] = ", round(E_L_encounter, digits=1))
    ...
    vlines!(ax, [E_L_encounter], color=:red, linestyle=:dash, linewidth=2, label="E[L]")
    if idx == 1
...
3. This uses the "encounter model" for edge-centric sampling:
   - Each sampled site (g, r) is an encounter opportunity
   - Expected edges: E[L] = E[N] × E[g·r]

   Alternative: The "pairing model" (sample_edge_centric) pairs source and target:
   - E[N]/2 edge opportunities
