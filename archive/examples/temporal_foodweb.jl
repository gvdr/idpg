...
"""
Sample edge-centric interactions from per-species grids with COUPLED (g, r) per species.

# Edge-Centric Semantics
Each edge opportunity "consumes" two node-equivalents (one source, one target).
From E[N] = Σ γ_m node-equivalents, we get E[N]/2 edge opportunities.

Expected edges: E[L] = (E[N]/2) · E[g·r]

Algorithm:
1. Compute species intensities γ_m = c_{G,m} · c_{R,m}
2. Compute E[N] = Σ γ_m (total node-equivalent intensity)
3. Sample L ~ Poisson(E[N]/2) edge opportunities
4. For each opportunity: sample source and target species, then sample (g, r) from each
5. Accept edge with probability g_source · r_target
"""
function sample_from_species_grids(ρ_G_species::Vector{Vector{Float64}},
    ...
    γ = c_G .* c_R
    E_N = sum(γ)  # Total node-equivalent intensity
